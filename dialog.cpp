...
void Dialog::getRowCol()
{
  ...
  else if (BOOMNUM >= ROW*COL) {
      QMessageBox message(QMessageBox::NoIcon, "warning", "设置雷数过多！");
      IsReal = false;
      message.exec();
  }
  else if(ROW>70 || COL>70 || BOOMNUM>((ROW*COL)/2) || BOOMNUM < 9) {
      QMessageBox message(QMessageBox::NoIcon, "warning", "请勿挑战奇怪的难度！");
      IsReal = false;
      message.exec();
  }
  else
