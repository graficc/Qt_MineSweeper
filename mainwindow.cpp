...
{
    int px=event->x()-offsetx;  //获取鼠标按下的坐标
    int py=event->y() -offsety;
    int m = (px) / 20;          //根据坐标计算出点到哪一格
    int n = (py) / 20 - 2;
...
void MainWindow::on_actionUser_Defined_triggered()
{
    ...
    if(dialog->IsReal)
    {
        mineMap.Create(dialog->ROW,dialog->COL,dialog->BOOMNUM);
        setFixedSize(mineMap.mx*20+offsetx*2,mineMap.my*20+offsety+66);
    }
    delete dialog;
}
...
void MainWindow::on_actionHow_to_play_triggered()
{
    ...
    message.setIconPixmap(QPixmap(":new/prefix1/res/baidu.bmp"));
    message.exec();
}
...
{
    QMessageBox message(QMessageBox::NoIcon, "thank you to see", "项目：MineSweeper\n班级：软1915\n作者：高佳晨\n博客：gapcolt.com");
    message.setIconPixmap(QPixmap(":new/prefix1/res/agree.bmp"));
    message.exec();
}
